package link.socket.ampere.cli.animation.agent
...
class AgentLayerRenderer(
    ...
    fun renderToGrid(layer: AgentLayer, gridWidth: Int, gridHeight: Int): List<String> {
        ...
        val grid = Array(gridHeight) { StringBuilder(" ".repeat(gridWidth)) }
        ...
        items.forEach { item ->
            ...
            if (y < gridHeight && x + nodeText.length <= gridWidth) {
                grid[y].replace(x, x + nodeText.length, nodeText)
            }
            ...
            item.statusDisplay?.let { status ->
                ...
                if (statusY < gridHeight) {
                    ...
                    if (x + statusText.length <= gridWidth) {
                        grid[statusY].replace(x, x + statusText.length, statusText)
                    }
                }
            }
        }

        return grid.map { it.toString() }
    }
