...
mavenPublishing {
    ...
    coordinates("link.socket", "ampere-bindings-apple", version.toString())

    pom {
        name.set("Ampere Bindings — Apple")
        description.set(
            "Apple platform-schema bindings for the Ampere canon: how each canon type " +
                "reaches Apple's assistant-schema vocabulary (AppIntents entity schemas and " +
                "system value types), keyed by CanonType via AppleCanonBindingRegistry.",
        )
...
kotlin {
    ...
    sourceSets {
        val commonMain by getting {
            dependencies {
                // Bindings project canon types onto Apple's assistant-schema vocabulary —
...
android {
    ...
    namespace = "link.socket.ampere.bindings.apple"
