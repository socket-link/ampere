...
- **A nullable `CanonId` cross-reference is a caller contract, not a resolution mechanism.** `CanonEmailMessage.mailboxId`, `CanonWorkItem.projectId`, `CanonMilestone.projectId`, and `CanonTable.documentId` all share this shape, and AMPR-266 settled it as the permanent answer rather than a placeholder for a future SPI:
