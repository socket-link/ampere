...
- **Marketplace rule: a published Arc or plug bundle names only `CanonType` members, and pins the minimum `ampereVersion` whose canon it names.** Extension types in published artefacts are forbidden — there is no namespace to put one in (see Anti-patterns) and no registry a marketplace could validate one against. The first half already holds by construction: `BundleManifest` embeds `PlugManifest` verbatim, whose `emits`/`consumes`/`optionalConsumes` are `Set<CanonType>`, so a bundle naming an unknown noun fails manifest decode and surfaces as `BundleParseError.InvalidManifest` instead of importing. The pin is the missing half. A bundle declares `bundleFormatVersion` and its own `plug.version`, but nothing that says which canon it was built against — so a bundle naming a noun admitted after the host shipped does fail, but it fails as `BundleParseError.InvalidManifest` out of the enum decode: the host tells the user the manifest is malformed when the truth is that the host is too old to read it. Pinning the minimum `ampereVersion` and checking it *before* decode is what turns that into the `UnknownVersion(declared, supported)` shape — the version-skew contract above, applied at import. That is the bundle-hardening follow-up; `docs/ampere/plug-bundle-format.md` is the format's source of truth and changes with it.
