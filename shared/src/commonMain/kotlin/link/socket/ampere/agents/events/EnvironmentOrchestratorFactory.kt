...
class EnvironmentOrchestratorFactory(
    ...
    private fun createTemporaryMeetingOrchestrator(
        meetingRepository: MeetingRepository,
        messageRepository: MessageRepository,
    ): link.socket.ampere.agents.events.meetings.MeetingOrchestrator {
        // Create a temporary message API for the orchestrator
        val tempMessageApi = link.socket.ampere.agents.events.messages.AgentMessageApi(
        ...
        return link.socket.ampere.agents.events.meetings.MeetingOrchestrator(
