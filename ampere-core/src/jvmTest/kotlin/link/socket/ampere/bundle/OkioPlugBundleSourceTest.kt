...
class OkioPlugBundleSourceTest {

    private val tempRoot = Files.createTempDirectory("plug-bundle-test")
    ...
    fun `fromDirectory parses a bundle laid out on disk`() {
        ...
        val source = PlugBundleSource.fromDirectory(bundleDir.toOkioPath(), FileSystem.SYSTEM)
        val ok = assertIs<BundleParseResult.Ok>(parser.parse(source))

        assertEquals(fixtureManifest.plug, ok.bundle.manifest)
    ...
    fun `fromDirectory on a missing path surfaces MissingManifest via parser`() {
        ...
        val source = PlugBundleSource.fromDirectory(ghost, FileSystem.SYSTEM)
    ...
    fun `fromZipFile parses a bundle stored as a ZIP archive`() {
        ...
        val source = PlugBundleSource.fromZipFile(zipFile.toOkioPath(), FileSystem.SYSTEM)
        val ok = assertIs<BundleParseResult.Ok>(parser.parse(source))
    ...
    fun `fromZipFile entries are keyed identically to fromDirectory`() {
        ...
        val dirEntries =
            PlugBundleSource.fromDirectory(bundleDir.toOkioPath(), FileSystem.SYSTEM).entries()
        val zipEntries =
            PlugBundleSource.fromZipFile(zipFile.toOkioPath(), FileSystem.SYSTEM).entries()
