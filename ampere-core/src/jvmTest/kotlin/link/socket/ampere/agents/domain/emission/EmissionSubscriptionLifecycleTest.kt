 ...
 *
 * jvmTest since AMPR-340: the bus-only `emission` overload is gone, so the scope publishes
 * through an [InMemoryEventApi] door, which needs a JDBC store. `runBlocking` rather than
 * `runTest`: the door's persist hops to `Dispatchers.IO`, and a virtual-time scheduler would
 * skip straight to the `withTimeout` deadlines while that hop is in flight.
 */
class EmissionSubscriptionLifecycleTest {
    ...
    private val source = EventSource.Agent(agentId)
    ...
    @Test
    fun `repeated emission calls leave the reply handler count unchanged`() = runBlocking {
        coroutineScope {
            val logger = CountingEventLogger()
            InMemoryEventApi.open(agentId = agentId, scope = this, logger = logger).use { handle ->
                val registry = EmissionReplyRegistry()

                repeat(3) { i ->
                    emission(source, handle.api, registry, principal = Principal.Ambient, parentEmissionId = null) {
    ...
    @Test
    fun `a reply after three completed calls reaches exactly one router`() = runBlocking {
        coroutineScope {
            val logger = CountingEventLogger()
            InMemoryEventApi.open(agentId = agentId, scope = this, logger = logger).use { handle ->
                val bus = handle.bus
    ...
    @Test
    fun `InputProvided on the bus resumes askHuman without a handler error`() = runBlocking {
        coroutineScope {
            val logger = CountingEventLogger()
            InMemoryEventApi.open(agentId = agentId, scope = this, logger = logger).use { handle ->
                val bus = handle.bus
    ...
    @Test
    fun `emission releases its reply handler when the block throws`() = runBlocking {
        coroutineScope {
            val logger = CountingEventLogger()
            InMemoryEventApi.open(agentId = agentId, scope = this, logger = logger).use { handle ->
    ...
    @Test
    fun `emission releases its reply handler when the caller is cancelled`() = runBlocking {
        coroutineScope {
            val logger = CountingEventLogger()
            InMemoryEventApi.open(agentId = agentId, scope = this, logger = logger).use { handle ->
                val bus = handle.bus
