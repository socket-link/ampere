...
class WorkspaceStateStoreTest {
    ...
    fun `fold TaskStarted transitions to InProgress with workspace`() {
        ...
        state = WorkspaceStateStore.fold(state, Event.TaskCreated(
            eventId = "evt-1",
            urgency = Urgency.MEDIUM,
            timestamp = now,
            eventSource = EventSource.Agent("agent-A"),
        ...
        state = WorkspaceStateStore.fold(state, TaskEvent.TaskStarted(
            eventId = "evt-2",
            taskId = "task-1",
    ...
    fun `fold TaskProgressed increments progress`() {
        ...
        state = WorkspaceStateStore.fold(state, TaskEvent.TaskProgressed(
            eventId = "evt-2",
            taskId = "task-1",
            eventSource = EventSource.Agent("agent-A"),
            timestamp = now,
    ...
    fun `fold TaskCompleted sets status and full progress`() {
        ...
        val completedAt = now
        state = WorkspaceStateStore.fold(state, TaskEvent.TaskCompleted(
            eventId = "evt-2",
            taskId = "task-1",
            eventSource = EventSource.Agent("agent-A"),
    ...
    fun `fold TaskFailed sets Blocked status with reason`() {
        ...
        state = WorkspaceStateStore.fold(state, TaskEvent.TaskFailed(
            eventId = "evt-2",
            taskId = "task-1",
            eventSource = EventSource.Agent("agent-A"),
            timestamp = now,
    ...
    fun `fold TaskBlocked adds to blockedBy set`() {
        ...
        state = WorkspaceStateStore.fold(state, TaskEvent.TaskBlocked(
            eventId = "evt-3",
            taskId = "task-2",
            eventSource = EventSource.Agent("agent-A"),
            timestamp = now,
    ...
    fun `fold SubtaskCreated adds child item with parentId`() {
        ...
        state = WorkspaceStateStore.fold(state, TaskEvent.SubtaskCreated(
            eventId = "evt-2",
            taskId = "task-1",
            eventSource = EventSource.Agent("agent-A"),
            timestamp = now,
    ...
    fun `fold ignores unrelated events`() {
        ...
        val result = WorkspaceStateStore.fold(state, Event.QuestionRaised(
            eventId = "evt-q",
            urgency = Urgency.LOW,
            timestamp = now,
            eventSource = EventSource.Agent("agent-A"),
    ...
    fun `fold ignores events for unknown task IDs`() {
        ...
        val result = WorkspaceStateStore.fold(state, TaskEvent.TaskStarted(
            eventId = "evt-1",
    ...
    fun `full task lifecycle through bus`() {
        runBlocking {
            ...
            // Progress
            bus.publish(TaskEvent.TaskProgressed(
                eventId = "evt-3",
                taskId = "task-1",
                eventSource = EventSource.Agent("agent-A"),
                timestamp = now,
